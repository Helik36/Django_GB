...
class ShopUserProfile(models.Model):
    ...
    @receiver(post_save, sender=ShopUser) # приёмщик сигнала. дальшнйшая функция Будет вызываться только тогда, когда какая-то модель будет сохранения из шопюзер
    def create_user_profile(sender, instance, created, **kwargs):
        if created: # проверяем, что он создаётся
